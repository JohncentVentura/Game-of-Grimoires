...
public class HUDPanelController : MonoBehaviour
{
    ...
    {   
        for (int i = 0; i < playerData.deckSize; i++)
        {
            ...
            cardButtons[i].manaSlider.value = playerData.activeDeck[i].GetCardStat(CARDSTATS.Mana).value;

            //CardButtons CooldownSlider value
            cardButtons[i].cooldownSlider.value = playerData.activeDeck[i].GetCardStat(CARDSTATS.Cooldown).value;

            //CardButtons CooldownText
            if (playerData.activeDeck[i].GetCardStat(CARDSTATS.Cooldown).value > 0)
            {
                cardButtons[i].cooldownText.text = "" + playerData.activeDeck[i].GetCardStat(CARDSTATS.Cooldown).value.ToString("F1"); //F1 means 1 decimal is shown
            }
            else
            {
            ...
            if (playerData.activeDeck[i].GetCardStat(CARDSTATS.Mana).value < playerData.activeDeck[i].GetCardStat(CARDSTATS.Mana).maxValue || playerData.activeDeck[i].GetCardStat(CARDSTATS.Cooldown).value > 0)
            {
            ...
            else if (playerData.activeDeck[i].GetCardStat(CARDSTATS.Mana).value >= playerData.activeDeck[i].GetCardStat(CARDSTATS.Mana).maxValue && playerData.activeDeck[i].GetCardStat(CARDSTATS.Cooldown).value <= 0)
            {
